...
class fc_input:
    ...
    def get_dcdz(self, maxlen, curve_height_setpoint, curve_nsamples, use_lifts):
        if use_lifts:
            if use_lifts:
                og_dim=self.og_dim
                lifts = [f for f in listdir(self.curves_path) if isfile(join(self.curves_path, f))]
                samples = [int(re.search(r'\d+', filename).group()) for filename in lifts]
                samples.sort()
                ...
                curves_sampled=lift_samples[:,:,1:]
                curves_sampled=matrix_cut(curves_sampled, self.nu_dim)
        else:
    ...
    def get_x(self, maxlen, curve_height_setpoint, curve_nsamples, see_data, use_lifts):
        
        self.get_dcdz(maxlen, curve_height_setpoint, curve_nsamples,use_lifts)
        self.get_vectors()
        ...
        self.x_mat=X_mat
        if see_data:
            max_v=(np.min(X_mat)+np.max(X_mat))/2+np.max(X_mat)/2
            min_v=(np.min(X_mat)+np.max(X_mat))/2-np.max(X_mat)/2
...
class preprocessing:
    ...
    def get_fc(self, maxlen=1713, curve_height_setpoint=20, curve_nsamples=12, see_data=0, use_lifts=0):
        if hasattr(self, 'mask_indexes'):
            fc = fc_input(self.topography_path, self.curves_path, self.y_path, self.cutout_size)
            fc.get_x(maxlen, curve_height_setpoint, curve_nsamples, see_data, use_lifts)
